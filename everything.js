...
document
.getElementById("priceFilter").addEventListener("change", function updatePriceFilter(val){
  val = this.value;
  document.getElementById("price-filter-input").value = " Below $" + val;
  ...
  for (let i = 0; i < productArray.length; i++) {
    ...
    if (p.price <= val) {
      let div_to_change = document.getElementById("ffb" + cnt);
      
      div_to_change.style.display="block";
      div_to_change.querySelector("a").href = p.url;
      div_to_change.querySelector("a > img").src = p.image;
      div_to_change.querySelector("a > p").innerHTML =
      p.name + " <br> " + "$" + p.price;
  ...
  for (let i = cnt; i < productArray.length; i++) {
    let div_to_change = document.getElementById("ffb" + i);
    div_to_change.style.display="none";
  }
  
});

document.getElementById("sortType").addEventListener("change",function updateSortList(val){
  ...
  if(val=="priceh"){
    ...
    for(let i=0;i<productArray.length;i++){
      let p=productArray[i];
      let div_to_change = document.getElementById("sfb" + i);
      div_to_change.querySelector("a").href = p.url;
      div_to_change.querySelector("a > img").src = p.image;
      div_to_change.querySelector("a > p").innerHTML =
      p.name + " <br> " + "$" + p.price;
...
document.getElementById("searchSubmit").addEventListener("click",function updateSearch(val){
  
  val=document.getElementById("psearch").value;
  
  for(let i=0;i<productArray.length;i++){
    let p=productArray[i];
    if(p.name==val){
      let div_to_change=document.getElementById("ffb0");
      div_to_change.querySelector("a").href = p.url;
      div_to_change.querySelector("a > img").src = p.image;
      div_to_change.querySelector("a > p").innerHTML =
      p.name + " <br> " + "$" + p.price;
...
function goto_checkout() {
  ...
  hide_cart(); // to hide cart when html loads
